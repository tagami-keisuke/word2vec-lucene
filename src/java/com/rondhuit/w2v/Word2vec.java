...
public class Word2vec {
  ...
  static {
    for (int i = 0; i < EXP_TABLE_SIZE; i++) {
      expTable[i] = Math.exp((i / (double) EXP_TABLE_SIZE * 2 - 1) * MAX_EXP); // Precompute
                                                                               // the
                                                                               // exp()
                                                                               // table
      ...
      try {
        ...
        while (true) {
          if (word_count - last_word_count > 10000) {
            ...
            System.err
                .printf(
                    "%cAlpha: %f  iter: %d  Progress: %.2f%%  Words/thread/sec: %.2fk  ",
                    13, alpha, local_iter, wordCountActual
                        / (float) (iter * trainWords + 1) * 100,
                    wordCountActual / (float) (timeNow - timeStart + 1));
            System.err.flush();
            alpha = startingAlpha
                * (1 - wordCountActual / (float) (iter * trainWords + 1));
          ...
          if (sentence_length == 0) {
            while (true) {
              ...
              if (sample > 0) {
                double ran = (Math.sqrt(vocab[word].cn / (sample * trainWords)) + 1)
                    * (sample * trainWords) / vocab[word].cn;
          ...
          if (word == -2 /* eof? */
              || (word_count > trainWords / numThreads)) {
            wordCountActual += word_count - last_word_count;
            local_iter--;
            if (local_iter == 0)
              break;
              ...
              if (negative > 0)
                for (d = 0; d < negative + 1; d++) {
                  if (d == 0) {
                    ...
                    next_random = nextRandom(next_random);
                    target = table[Math
                        .abs((int) ((next_random >> 16) % TABLE_SIZE))];
                    if (target == 0)
                      target = Math
                          .abs((int) (next_random % (vocabSize - 1) + 1));
                    if (target == word)
                      continue;
                    label = 0;
                  }
                  ...
                  if (f > MAX_EXP)
                    ...
                    g = (label - expTable[(int) ((f + MAX_EXP) * (EXP_TABLE_SIZE
                        / MAX_EXP / 2))])
                        * alpha;
                  for (c = 0; c < layer1Size; c++)
  ...
  public WordVectors trainModel(CorpusFactory corpusFactory)
    ...
    for (int i = 0; i < config.getNumThreads(); i++) {
      new TrainModelThread(this, corpusFactory.create(corpus), config, i)
          .start();
    }
    synchronized (this) {
      while (threadCount > 0) {
  ...
  void initNet(Corpus corpus) {
    ...
    for (int i = 0; i < vocabSize; i++) {
      for (int j = 0; j < layer1Size; j++) {
        ...
        syn0[i * layer1Size + j] = (((nextRandom & 0xFFFF) / (double) 65536) - 0.5)
            / layer1Size;
      }
    }
