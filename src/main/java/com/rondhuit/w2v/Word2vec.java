...
public class Word2vec {
    ...
    static class TrainModelThread extends Thread {
        ...
        public TrainModelThread(Word2vec vec, Corpus corpus, Config config,
                int id) {
            this.vec = vec;
            this.corpus = corpus;
            this.config = config;
            ...
            try {
                ...
                while (true) {
                    if (word_count - last_word_count > 10000) {
                        ...
                        System.err
                                .printf("%cAlpha: %f  iter: %d  Progress: %.2f%%  Words/thread/sec: %.2fk  ",
                                        13,
                                        alpha,
                                        local_iter,
                                        wordCountActual
                                                / (float) (iter * trainWords + 1)
                                                * 100,
                                        wordCountActual
                                                / (float) (timeNow - timeStart + 1));
                        System.err.flush();
                        alpha = startingAlpha
                                * (1 - wordCountActual
                                        / (float) (iter * trainWords + 1));
                    ...
                    if (sentence_length == 0) {
                        while (true) {
                            ...
                            if (sample > 0) {
                                double ran = (Math.sqrt(vocab[word].cn
                                        / (sample * trainWords)) + 1)
                                        * (sample * trainWords)
                                        / vocab[word].cn;
                                next_random = nextRandom(next_random);
                                if (ran < (next_random & 0xFFFF)
                                        / (double) 65536)
                                    continue;
                            }
